...
milsa$Grau_Instrucao = factor(milsa$Grau_Instrucao, ordered = T)
```
...
```{r}
civil_tb_prob = prop.table(civil_tb)
civil_tb_prob
```
...
```{r}
graf = ggplot(milsa, aes(x = Estado_Civil)) + geom_bar()
graf
```
...
```{r}
graf2 = ggplot(milsa, aes(x = Grau_Instrucao)) + geom_bar()
graf2
```
...
```{r}
ggplot(milsa, aes(x = Salario)) + geom_histogram(bins = 10)
```
...
```{r}
ggplot(milsa, aes(x = Idade, y = Salario)) + geom_point()
```
